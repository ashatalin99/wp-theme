 ...
 */
function custom_template_query_vars() {
	global $wp;
	$wp->add_query_var('custom');
}
add_action('init', 'custom_template_query_vars');

/**
 * Adds custom rules to wp_rewrite for pretty URLs
 */
function custom_template_rewrite_rules( $rules ) {
	$custom['custom/(.+)$'] = 'index.php?custom=$matches[1]';
	return $custom + $rules;
}
add_filter('rewrite_rules_array', 'custom_template_rewrite_rules');
